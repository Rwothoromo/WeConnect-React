...
describe('<ShowBusiness />', () => {
	...
	it('shows a businesses', async () => {
		...
		const BusinessComponent = wrapper.find(ShowBusiness)

		mock.onGet(`${apiUrl}/businesses/2/reviews`).reply(200, {
			reviews_list: [{}, {}]
		});

		let spyBusiness = jest.spyOn(BusinessComponent.instance(), 'componentDidMount');
		await BusinessComponent.instance().componentDidMount();
		expect(spyBusiness).toHaveBeenCalled();
	});
});
